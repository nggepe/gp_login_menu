...
<div class="main-content">
    <section class="section">
        ...
        <div class="section-body">
	        <h2 class="section-title">Halaman manajemen produk</h2>
	        <p class="section-lead">Halaman ini digunakan untuk mengelola data produk.</p>
	        <div class="card">
	        	<div class="card-header">
	                <h4>Tabel data produk</h4>
	            </div>
	            
	            <div class="card-body">
	                <button class="btn btn-md btn-info" id="btn-tambah"><i class="fa fa-plus"></i> Tambah data produk</button>
	                <hr>
...
$(document).ready(function(){
    table = $('#ajax_table').DataTable({
        "order": [],
        "ajax": {
            "url": "<?php echo base_url('generated/produk/ajaxTable');?>",
            "type": "POST"
        },
        "columnDefs": [
            {
...
$("#btn-simpan").click(function(){
    ...
    if(status =="true") {
        if (save_method=="add") {
            url = "<?= base_url() ?>generated/produk/ajax_save";
        }
        else{
            url = "<?= base_url() ?>generated/produk/ajax_update/"+global_id;
        }
        $.ajax({
            url : url,
            type: "POST",
...
function edit(id)
{
    ...
    save_method = "edit";
    $.ajax({
        url : "<?php echo base_url(); ?>generated/produk/ajax_edit/"+id,
        type: "POST",
        dataType: "JSON",
        success: function(data)
        {
...
function hapus(id)
{
    swal({   
        ...
        if (isConfirm) {
            ...
            $.ajax({
                url : "<?php echo base_url(); ?>generated/produk/ajax_delete/"+id,
                type: "POST",
                dataType: "JSON",
                success: function(data)
                {
